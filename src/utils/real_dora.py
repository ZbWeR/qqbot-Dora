...
class DoraLifeBot:
    ...
    def speak(self,msg_keywords):
        ...
        if result:
            ans_array = result["answer"]
            new_list = [item for item in ans_array if item["count"] >= 3]
            if len(new_list)==0:
                return "SILENT"
    ...
    def Mewo(self,message,uid,gid):
        ...
        if opt<=1:
            ...
            return self.speak(now_keywords)
        else:
            return "SILENT"
